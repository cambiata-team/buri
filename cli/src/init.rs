...
pub fn do_init(root: &VfsPath, name: &Option<String>) -> Result<(), VfsError> {
    let workspace_file = root.join("WORKSPACE")?;
    if workspace_file.exists()? {
        println!("Workspace already exists, no need to create a new one.");
        return Ok(());
    }
    ...
    println!("Created new workspace at .{}", workspace_file.as_str());
    Ok(())
}
...
mod test {
    ...
    fn creates_workspace_file_with_name() {
        let root: VfsPath = MemoryFS::new().into();
        do_init(&root, &Some("foo".to_string())).unwrap();
        let workspace_file = root.join("WORKSPACE").unwrap();
